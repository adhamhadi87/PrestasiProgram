...
def build_bahagian_chart(filtered_df, bahagian_col, chart_height=None):
    ...
    Hanya angka/nilai dalam bar dijadikan clickable.
    Klik pada bar biasa tidak akan membuka senarai.
    """
    bahagian_status = (
        filtered_df
    ...
    # Bar hanya visual; jangan proses klik bar.
    fig_bahagian_stack.update_traces(
        text=None,
        hovertemplate="<b>%{y}</b><br>Jumlah: %{x}<extra></extra>"
    )

    # Bina kedudukan tengah setiap segment untuk letak angka clickable.
    text_rows = []
    for bahagian, group in bahagian_status.groupby(bahagian_col, sort=False):
        cumulative = 0
        group = group.sort_values("KATEGORI_TRAFFIC")

        for _, row in group.iterrows():
            jumlah = row["JUMLAH"]
            status = str(row["KATEGORI_TRAFFIC"])

            if pd.notna(jumlah) and jumlah > 0:
                midpoint = cumulative + (jumlah / 2)
                text_rows.append({
                    bahagian_col: bahagian,
                    "KATEGORI_TRAFFIC": status,
                    "JUMLAH": jumlah,
                    "MIDPOINT": midpoint
                })
    ...
    if not text_df.empty:
        for status in status_order:
            sub = text_df[text_df["KATEGORI_TRAFFIC"] == status].copy()

            if sub.empty:
                continue

            fig_bahagian_stack.add_trace(
                go.Scatter(
                    x=sub["MIDPOINT"],
                    y=sub[bahagian_col],
                    mode="text",
                    text=sub["JUMLAH"].astype(int).astype(str),
                    textposition="middle center",
                    customdata=sub["KATEGORI_TRAFFIC"],
                    name=f"CLICK_NILAI_{status}",
                    showlegend=False,
                    hovertemplate=(
                        "<b>Klik nilai ini</b><br>"
                        "Bahagian: %{y}<br>"
...
if st.session_state.fullscreen_chart:
    ...
    if clicked_fullscreen:
        ...
        if clicked_curve is not None and clicked_bahagian is not None:
            trace_name = fig_fullscreen.data[clicked_curve].name

            # Hanya angka/nilai text trace boleh trigger senarai.
            # Klik pada bar biasa tidak akan diproses.
            if str(trace_name).startswith("CLICK_NILAI_"):
                clicked_status = str(trace_name).replace("CLICK_NILAI_", "")
...
with st.expander("📊 PENCAPAIAN MENGIKUT BAHAGIAN", expanded=False):
    ...
    st.caption("Klik angka/nilai di dalam bar sahaja untuk lihat senarai program bagi status tersebut.")
    ...
    if clicked_chart:
        ...
        if clicked_curve is not None and clicked_bahagian is not None:
            trace_name = fig_bahagian_stack.data[clicked_curve].name
